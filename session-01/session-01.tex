...
% Use th    \concept{Illustrative Ex    \begin{center}
        \Large\textbf{Differential equations constitute the fundamental language of change!}
    \end{center}les:}
    \begin{align}
        \frac{dy}{dx} &= 3x + 2 && \text{(First-order linear)} \\[0.2cm]
        \frac{d^2y}{dx^2} + 4y &= 0 && \text{(Second-order homogeneous)} \\[0.2cm]
        \frac{\partial u}{\partial t} &= \frac{\partial^2 u}{\partial x^2} && \text{(Partial differential equation)}
    \end{align}opolis theme
\usetheme{metropolis}
